...
public class CalendarCard extends View {
	...
	//private String[] signdate={"2016,8,1","2016,8,2"};
	private String[] signdate;
	private Mydata mydata=new Mydata(this.getContext());
	...
	private void fillDate() {
		...
		signdate=mydata.getData("signinfo").split(";");
		System.out.println(signdate[0]);
		...
		for (int j = 0; j < TOTAL_ROW; j++) {
			...
			for (int i = 0; i < TOTAL_COL; i++) {
				...
				if (position >= firstDayWeek
					...
					// 今天
					String judgedate=mShowDate.getYear()+","+mShowDate.getMonth()+","+day;
					if (JudgeSign(judgedate)) {
	...
	private boolean JudgeSign(String judgedate) {
		// TODO Auto-generated method stub
		for(int i=0;i<signdate.length;i++){
			if(signdate[i].equals(judgedate)){
				return true;
			}
		}
		return false;
